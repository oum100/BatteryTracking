...
interface BatteryJobRecord {
  ...
  phase: JobPhase
  status: string
  workflowStage?: 'NEW_JOB' | 'BEFORE_CHARGING' | 'AFTER_CHARGING' | 'QC_FOR_DELIVERY' | 'SHIPPED'
...
const employeeSelectOptions = computed(() => employees.value.map(employee => ({
  label: `${employee.code} - ${employee.name}`,
  value: employee.id,
  code: employee.code,
  name: employee.name,
...
const chargeChannelOptions = computed(() => chargeChannels.value.map(channel => ({
  label: channel.name,
  value: channel.id,
})))
const chargeProgramOptions = computed(() => chargePrograms.value.map(program => ({
  label: program.name,
  value: program.id,
})))
...
const selectedPendingAdminJobOption = computed(() => pendingAdminJobOptions.value
  .find(job => job.value === adminJobId.value) ?? null)
function getDefaultNightChargeProgramId() {
  const nightProgram = chargePrograms.value.find(program =>
...
function getPhaseOperatorId(job: BatteryJobRecord | null, targetPhase: JobPhase | null) {
  if (!job) {
    return ''
  }
...
function sanitizeBatteryIdInput(value: string) {
  const mapped = mapThaiKeyboardToAscii(String(value ?? '').normalize('NFKC').trim())
  return mapped
    .toUpperCase()
    .replace(/\s+/g, '')
    .replace(/[^A-Z0-9\-_/.:]/g, '')
...
function getFocusableInput(id: string) {
  ...
  return target.querySelector('input, textarea, button, [role="combobox"]') as HTMLInputElement | HTMLTextAreaElement | HTMLButtonElement | null
}

function focusSelectableElement(element: HTMLInputElement | HTMLTextAreaElement | HTMLButtonElement | null) {
  if (!element) {
    return
  }
...
const jobHasRecordedSlotData = computed(() => Boolean(
  currentJob.value?.slots.some(slotHasRecordedData),
))
...
const jobDetailsLocked = computed(() => hasJob.value && (Boolean(currentJob.value?.isLocked) || currentPhaseConfirmed.value))
const rackDetailsLocked = computed(() => jobDetailsLocked.value || jobHasRecordedSlotData.value)
const jobDetailsMatchCurrentJob = computed(() => {
  ...
  if ((operatorId.value || '') !== getPhaseOperatorId(currentJob.value, phase.value)) {
    return false
  }

  if (phase.value === 'BEFORE_CHARGE') {
    return adminJobId.value === currentJob.value.id
...
const hasPersistedRequiredJobDetails = computed(() => Boolean(
  currentJob.value
  && currentJob.value.hasAssignedRack !== false
  && currentJob.value.rackId.trim()
  && getPhaseOperatorId(currentJob.value, phase.value)
  && (phase.value !== 'BEFORE_CHARGE' || (currentJob.value.chargeChannelId && currentJob.value.chargeProgramId)),
))
const showRackLayoutCard = computed(() => hasJob.value && (
...
const hasRackInputReady = computed(() => Boolean(
  rackId.value.trim(),
))
const hasEditableJobDetailsChanges = computed(() => {
  ...
  const operatorChanged = (operatorId.value || '') !== getPhaseOperatorId(currentJob.value, phase.value)
...
const canSaveJobDetails = computed(() => {
  ...
  if (phase.value === 'BEFORE_CHARGE') {
    return Boolean(adminJobId.value && chargeChannelId.value && chargeProgramId.value && hasEditableJobDetailsChanges.value)
  }

  return hasEditableJobDetailsChanges.value
})
const canStartWorkflow = computed(() => Boolean(currentJob.value && hasRackAssignedForWorkflow.value))
const canStartBatteryWorkflow = computed(() => canStartWorkflow.value && isBatteryIdWorkflowEnabled.value)
...
function syncModalSlotInputs() {
  ...
  const phaseVoltage = modalSelectedSlot.value && phase.value
...
function openMeasurementPopup(duration = FEEDBACK_POPUP_DURATION) {
  ...
  const reopen = () => {
    ...
    if (typeof window !== 'undefined') {
      window.requestAnimationFrame(() => window.scrollTo({ left: scrollLeft, top: scrollTop, behavior: 'instant' }))
    }
...
function fillCalibrationEspInput(point: 1 | 2) {
  const fallbackVoltage = lastEspVoltReading.value ?? selectedSlot.value?.beforeVoltage ?? selectedSlot.value?.afterVoltage ?? selectedSlot.value?.deliveryVoltage ?? null
...
async function focusActiveModalField() {
  ...
  const activeMode = workflowActionMode.value ?? getDefaultWorkflowActionMode(phase.value ?? 'BEFORE_CHARGE')
...
function handleEmployeeSelection(employeeId: string | { value?: string, label?: string } | null) {
  const normalized = typeof employeeId === 'object' && employeeId !== null
...
async function handleBatteryInput(value: string) {
  ...
  if (!isLikelyBatteryIdToken(batteryScanInput.value)) {
    actionMessage.value = 'ค่าที่อ่านมาเป็นรูปแบบ Voltage หรือ Battery ID สั้นเกินไป'
    return
  }
...
async function startBatteryIdScanWorkflow() {
  ...
  if (!hasRackAssignedForWorkflow.value) {
    actionMessage.value = 'บันทึกข้อมูล Job Details ก่อน แล้วค่อยเริ่มอ่าน Battery ID'
    return
  }
...
async function startVoltageWorkflow() {
  ...
  if (!hasRackAssignedForWorkflow.value) {
    actionMessage.value = 'บันทึกข้อมูล Job Details ก่อน แล้วค่อยเริ่มอ่าน Voltage'
    return
  }
...
function normalizeJob(job: any): BatteryJobRecord {
  ...
  return {
    ...
    hasAssignedRack: typeof job.hasAssignedRack === 'boolean' ? job.hasAssignedRack : undefined,
    rackLabel: job.rackLabel ? String(job.rackLabel) : undefined,
    phase: job.phase as JobPhase,
    status: job.status,
    workflowStage: job.workflowStage ? String(job.workflowStage) as BatteryJobRecord['workflowStage'] : undefined,
    workflowLabel: job.workflowLabel ? String(job.workflowLabel) : undefined,
    recommendedPhase: job.recommendedPhase ? String(job.recommendedPhase) as JobPhase : undefined,
    ...
    beforeChargeOperatorId: job.beforeChargeOperatorId ? String(job.beforeChargeOperatorId) : null,
    beforeChargeOperatorName: job.beforeChargeOperatorName ? String(job.beforeChargeOperatorName) : null,
    afterChargeOperatorId: job.afterChargeOperatorId ? String(job.afterChargeOperatorId) : null,
    afterChargeOperatorName: job.afterChargeOperatorName ? String(job.afterChargeOperatorName) : null,
    beforeDeliveryOperatorId: job.beforeDeliveryOperatorId ? String(job.beforeDeliveryOperatorId) : null,
    beforeDeliveryOperatorName: job.beforeDeliveryOperatorName ? String(job.beforeDeliveryOperatorName) : null,
    ...
    plannedDeliveryDate: job.plannedDeliveryDate ? String(job.plannedDeliveryDate) : null,
...
function applyJob(job: any) {
  ...
  chargeChannelId.value = currentJob.value.chargeChannelId ?? ''
  chargeProgramId.value = currentJob.value.chargeProgramId ?? getDefaultNightChargeProgramId()
...
async function applyPendingAdminJob(jobSelection?: string | { value?: string, label?: string } | null) {
  const normalized = typeof jobSelection === 'object' && jobSelection !== null
    ? String(jobSelection.value ?? jobSelection.label ?? '').trim()
    : String(jobSelection ?? '').trim()
  const match = pendingAdminJobOptions.value.find(job => job.value === normalized || job.label === normalized)
  ...
  if (hasPersistedRequiredJobDetails.value) {
    ...
    const workflowLabel = workflowActionMode.value === 'battery' ? 'Battery ID' : 'Voltage'
...
function selectSlot(slotNumber: number) {
  ...
  if (!workflowActionMode.value) {
    workflowActionMode.value = getDefaultWorkflowActionMode(phase.value ?? 'BEFORE_CHARGE')
  }
...
async function loadMasters() {
  const [employeeResponse, salesOrderResponse, invoiceResponse, chargeChannelResponse, chargeProgramResponse, jobResponse] = await Promise.all([
    ...
    $fetch<{ jobs: BatteryJobRecord[] }>('/api/battery-jobs'),
  ])
...
async function openCurrentJob(skipRequiredValidation = false) {
  ...
  if (!currentJob.value) {
    actionMessage.value = 'หน้านี้ใช้สำหรับทำงานจากใบงานที่ Admin สร้างไว้แล้ว กรุณาโหลด Rack ก่อน'
    return
  }
  ...
  if (!skipRequiredValidation && phase.value === 'BEFORE_CHARGE' && !chargeChannelId.value) {
    actionMessage.value = 'QC Before Charge ต้องเลือก Charging Channel ก่อน'
    return
  }

  if (!skipRequiredValidation && phase.value === 'BEFORE_CHARGE' && !chargeProgramId.value) {
...
async function saveSelectedBatteryId() {
  ...
  try {
    ...
    const duplicateSlot = checkDuplicateBatteryId.value
      ? currentJob.value.slots.find(slot => (
    ...
    applyJob(response.job)
    const nextSlot = currentJob.value?.slots.find(slot => !slot.batteryId.trim()) ?? null
    ...
    if (!detailModalOpen.value) {
      if (nextSlot) {
        ...
        workflowActionMode.value = 'voltage'
        selectedSlotNumber.value = currentJob.value?.slots.find(slot => getPhaseVoltage(slot, phase.value) === null)?.slotNumber ?? 1
        syncWorkflowSlotInputs()
        actionMessage.value = 'บันทึก Battery ID ครบทุก slot แล้ว พร้อมวัด Voltage ที่ slot 1'
        await restoreWorkflowFocus()
      }
      return
    }
...
async function saveActiveSlotVoltage(voltageValue: string) {
  ...
  try {
    ...
    const nextSlot = currentJob.value?.slots.find(slot => getPhaseVoltage(slot, phase.value) === null) ?? null
    await wait(FEEDBACK_POPUP_DURATION)
    actionMessage.value = nextSlot
...
async function confirmCurrentPhase() {
  ...
  try {
    ...
    applyJob(response.job)
    actionMessage.value = confirmedPhase === 'DELIVERY'
  ...
  <main class="min-h-screen bg-[linear-gradient(135deg,_#eef7e6_0%,_#e8f0ff_50%,_#fff4dc_100%)] px-4 py-3 text-slate-950">
    <section class="mx-auto flex max-w-[1600px] flex-col gap-3">
      <template v-if="hasPhaseSelected">
          ...
          <div class="grid items-center gap-4 px-5 py-4 lg:grid-cols-[minmax(0,1fr)_auto_minmax(0,1fr)] lg:px-6 lg:py-4">
            <div class="flex items-center gap-4">
              ...
              <div>
                ...
                <div class="mt-1 text-xs font-semibold text-white/85 lg:text-sm">
        ...
        <div ref="jobDetailsCard">
          ...
          <div class="flex items-center justify-between gap-3">
            <div class="text-xs font-bold uppercase tracking-[0.2em] text-slate-500">{{ currentPhaseJobDetailsTitle }}</div>
            <div class="flex items-center justify-end gap-3">
              <div v-if="jobDetailsCollapsed" class="text-xs font-semibold text-slate-500">
                Workflow Active
              </div>
              <UButton
          ...
          <template v-if="jobDetailsCollapsed">
            <div class="mt-3 grid gap-2 xl:grid-cols-6">
              <UInput :model-value="currentJob?.jobRef || '-'" readonly color="neutral" variant="ghost" size="md" class="w-full" :ui="qcReadonlyUi" />
              <UInput :model-value="currentJob?.rackId || '-'" readonly color="neutral" variant="ghost" size="md" class="w-full" :ui="qcReadonlyUi" />
              <UInput :model-value="selectedEmployee?.code || '-'" readonly color="neutral" variant="ghost" size="md" class="w-full" :ui="qcReadonlyUi" />
              <UInput :model-value="selectedEmployee?.name || '-'" readonly color="neutral" variant="ghost" size="md" class="w-full" :ui="qcReadonlyUi" />
              <UInput :model-value="currentJob?.chargeChannelName || '-'" readonly color="neutral" variant="ghost" size="md" class="w-full" :ui="qcReadonlyUi" />
              <UInput :model-value="currentJob?.chargeProgramName || '-'" readonly color="neutral" variant="ghost" size="md" class="w-full" :ui="qcReadonlyUi" />
            </div>
          </template>

          <template v-else-if="phase === 'BEFORE_CHARGE'">
            <div class="mt-4 grid gap-3 xl:grid-cols-[1.15fr_repeat(4,minmax(0,1fr))]">
              <UFormField label="QC Job" name="admin-qc-job" required :ui="qcFieldUi">
                <div id="qc-job-input">
                  <USelectMenu
                    v-model="adminJobId"
                  ...
                  >
                    <template #default>
                      ...
                      <span v-else class="text-slate-400">เลือกใบงานที่ Admin สร้างไว้</span>
                    </template>
            ...
            <div class="mt-3 grid gap-3 xl:grid-cols-[1.15fr_repeat(4,minmax(0,1fr))]">
              ...
              <UFormField label="Emp ID" name="employee-id" required :ui="qcFieldUi">
                <div id="qc-employee-input">
                  ...
                  >
                    <template #default>
                      <span v-if="selectedEmployeeOption" class="block truncate">
                        <span class="font-black text-slate-950">{{ selectedEmployeeOption.code }}</span>
                        <span class="ml-2 text-slate-500">{{ selectedEmployeeOption.name }}</span>
                      </span>
                      <span v-else class="text-slate-400">เลือก Emp ID</span>
                    </template>

                    <template #item-label="{ item }">
                      <span class="font-black text-slate-950">{{ item.code }}</span>
              ...
              <UFormField label="Employee Name" name="employee-name" :ui="qcFieldUi">
                <UInput
              ...
              <UFormField label="Charge Channel" name="charge-channel" required :ui="qcFieldUi">
                <USelectMenu
              ...
              <UFormField label="Charge Program" name="charge-program" required :ui="qcFieldUi">
                <USelectMenu
            ...
            <div v-else-if="rackDetailsLocked" class="mt-3 rounded-[14px] border border-sky-300 bg-sky-50 px-4 py-3 text-sm font-semibold text-sky-950">
              Rack # และ QC Job ถูกล็อกแล้วหลังเริ่มบันทึก slot แต่ยังแก้ Emp ID, Charge Channel และ Charge Program ได้จนกว่าจะ Confirm
            </div>
          </template>
            ...
            <div class="flex flex-wrap items-center justify-end gap-3">
              <div class="rounded-[14px] border border-slate-200 bg-slate-50 px-4 py-3">
                <div class="flex flex-wrap items-center gap-4">
                  ...
                  <label class="flex cursor-pointer items-center gap-2 whitespace-nowrap text-xs font-black text-slate-700">
              ...
              <div class="rounded-[14px] border border-slate-200 bg-white px-4 py-3 shadow-sm">
                <div class="text-xs font-bold uppercase tracking-[0.16em] text-slate-500">Voltage {{ progressCount }} / 21</div>
                <UButton
                  color="neutral"
                  variant="solid"
        ...
        >
          <div class="rounded-[22px] border border-dashed border-slate-300 bg-slate-50 px-6 py-12 text-center">
            <div class="text-lg font-black text-slate-900">Rack Layout จะเปิดหลังบันทึก Job Details</div>
            <div class="mt-2 text-sm text-slate-600">
              กรอกข้อมูลที่จำเป็นให้ครบแล้วกด Save เพื่อแสดง 21 slot ของ rack นี้
            </div>
          </div>
        </UCard>
      ...
      <UModal
        ...
        :dismissible="true"
        :close="false"
        :content="{ class: 'w-[min(68vw,920px)] max-w-none rounded-[24px] bg-white shadow-[0_24px_80px_rgba(15,23,42,0.28)]' }"
        ...
        <template #body>
          <div class="space-y-5">
            <div class="flex items-start justify-between gap-3">
              <div>
                <div class="text-xs font-bold uppercase tracking-[0.2em] text-slate-500">ESP32 Calibration</div>
                <div class="mt-1 text-3xl font-black text-slate-950">INA226 Gain / Offset</div>
                <div class="mt-2 text-sm text-slate-600">
                  เทียบค่าแรงดัน 2 จุด ระหว่างมิเตอร์จริงกับ ESP Volt Meter แล้วส่งค่ากลับไปเก็บที่ ESP32
                </div>
              </div>
              <UButton color="neutral" variant="ghost" icon="i-lucide-x" class="rounded-full" @click="calibrateModalOpen = false" />
            </div>

            <div class="grid gap-4 lg:grid-cols-[minmax(0,1.2fr)_minmax(0,1fr)]">
              <UCard :ui="{ root: 'rounded-[18px] ring-0 bg-slate-50 shadow-[0_8px_20px_rgba(15,23,42,0.06)]', body: 'p-4' }">
                <div class="flex items-start justify-between gap-3">
                  <div>
                    <div class="text-xs font-bold uppercase tracking-[0.2em] text-slate-500">Device Sync</div>
                    <div class="mt-2 text-sm font-semibold" :class="bleConnected ? 'text-emerald-700' : 'text-rose-600'">
                      {{ calibrationBleStatusLabel }}
                    </div>
                    <div v-if="!bleConnected" class="mt-1 text-xs text-slate-500">
                      การ pair แบบคีย์บอร์ดหรือ HID ของระบบ ยังใช้คาลิเบรตไม่ได้ ต้องเชื่อมต่อ BLE จากหน้านี้อีกครั้ง
                    </div>
                    ...
                    <div class="mt-2 text-sm text-slate-500">
                      Device: <span class="font-bold text-slate-900">{{ bleMeterDeviceId || bleDeviceName }}</span>
                    </div>
                    <div class="mt-1 text-sm text-slate-500">
                      Firmware:
                      <span class="font-bold text-slate-900">{{ bleMeterFirmwareVersion || '-' }}</span>
                    </div>
                  </div>
              ...
              <UCard :ui="{ root: 'rounded-[18px] ring-0 bg-slate-950 text-white shadow-[0_14px_32px_rgba(15,23,42,0.18)]', body: 'p-4' }">
                <div class="text-xs font-bold uppercase tracking-[0.2em] text-emerald-200">Calculated Result</div>
                <div class="mt-3 text-lg font-black">{{ calibrateResultLabel }}</div>
                ...
                <div class="mt-4 grid grid-cols-2 gap-3 text-sm">
                  <div class="rounded-xl bg-white/10 px-3 py-3">
                    <div class="text-xs font-bold uppercase tracking-[0.18em] text-white/60">Gain</div>
                    <div class="mt-1 text-xl font-black">{{ calibrateResultGain !== null ? calibrateResultGain.toFixed(5) : '-' }}</div>
                  </div>
                  <div class="rounded-xl bg-white/10 px-3 py-3">
                    <div class="text-xs font-bold uppercase tracking-[0.18em] text-white/60">Offset</div>
                    <div class="mt-1 text-xl font-black">{{ calibrateResultOffset !== null ? `${calibrateResultOffset >= 0 ? '+' : ''}${calibrateResultOffset.toFixed(5)}` : '-' }}</div>
                  </div>
                </div>
              </UCard>
            </div>
            ...
            <div class="grid gap-4 lg:grid-cols-2">
              <UCard :ui="{ root: 'rounded-[18px] ring-0 bg-white shadow-[0_10px_28px_rgba(15,23,42,0.06)]', body: 'p-4' }">
                ...
                <div class="mt-4 grid gap-4 md:grid-cols-2">
                  <UFormField label="Meter Voltage" name="cal-point-one-meter" :ui="qcFieldUi">
                    <UInput
                  ...
                  <UFormField label="ESP Volt Meter" name="cal-point-one-esp" :ui="qcFieldUi">
                    <UInput
              ...
              <UCard :ui="{ root: 'rounded-[18px] ring-0 bg-white shadow-[0_10px_28px_rgba(15,23,42,0.06)]', body: 'p-4' }">
                ...
                <div class="mt-4 grid gap-4 md:grid-cols-2">
                  <UFormField label="Meter Voltage" name="cal-point-two-meter" :ui="qcFieldUi">
                    <UInput
                  ...
                  <UFormField label="ESP Volt Meter" name="cal-point-two-esp" :ui="qcFieldUi">
                    <UInput
            ...
            <div class="flex items-center justify-between gap-3 rounded-[18px] border border-slate-200 bg-slate-50 px-4 py-3">
            ...
            <div class="space-y-4">
              <div class="grid grid-cols-[1fr_auto] items-start gap-4">
                <div class="text-center">
                  <div class="text-sm font-bold uppercase tracking-[0.24em] text-slate-500">Active Slot</div>
                  <div class="mt-2 text-5xl font-black tracking-tight text-slate-950">
                    Slot {{ editingSlotNumber ?? selectedSlotNumber }}
                  </div>
                </div>
                <div class="flex items-start justify-self-end gap-2">
                  <UButton color="neutral" variant="ghost" icon="i-lucide-x" class="rounded-full" @click="closeSlotDetail" />
                </div>
              </div>
            </div>

            <div class="grid gap-4 xl:grid-cols-2">
              <UFormField label="Battery ID" name="slot-battery-id" :ui="qcFieldUi">
                <UInput
                  id="qc-battery-input"
                  v-model="batteryScanInput"
                  type="text"
      ...
      >
        <div class="min-w-[320px] max-w-[min(42vw,680px)] rounded-[18px] bg-slate-950/96 px-8 py-6 text-center text-white shadow-[0_30px_80px_rgba(15,23,42,0.5)] ring-1 ring-white/15">
          <div class="text-sm font-bold uppercase tracking-[0.22em] text-emerald-200">{{ measurementPopupLabel }}</div>
          <div v-if="measurementPopupContext" class="mt-1 text-lg font-black text-white/80">
            {{ measurementPopupContext }}
          </div>
          <div class="mt-3 text-5xl font-black tracking-tight">
            {{ measurementPopupValue }}
          </div>
          <div v-if="measurementPopupUnit" class="mt-2 text-xl font-bold text-emerald-100">
